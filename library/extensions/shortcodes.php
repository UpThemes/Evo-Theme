...
function thmfooter_theme_link() {
    ...
    return apply_filters('evo_theme_link',$themelink);
}
add_shortcode('theme-link', 'thmfooter_theme_link');	

function thmfooter_login_link() {
    if ( ! is_user_logged_in() )
        $link = '<a href="' . get_option('siteurl') . '/wp-login.php">' . __('Login','evo') . '</a>';
    else
    $link = '<a href="' . wp_logout_url($redirect) . '">' . __('Logout','evo') . '</a>';
